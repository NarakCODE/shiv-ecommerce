...
export default function HomeClient({ initialProducts }: Props) {
  ...
  const [currentItems, setCurrentItems] = useState(initialProducts);
  ...
  const handleSeemore = async (
    ...
    if (backendUrl) {
      try {
        ...
        console.warn("Could not fetch more products:", err);
      }
    }
    setIsFetching(false);
  };
      ...
      <main id="main-content" className="-mt-20">
        ...
        <section className="app-max-width w-full h-full flex flex-col justify-center mt-16 mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 lg:gap-x-12 gap-y-6 mb-10 app-x-padding">
            ...
            {currentItems[3] && <Card key={currentItems[3].id} item={currentItems[3]} />}
            {currentItems[4] && <Card key={currentItems[4].id} item={currentItems[4]} />}
          </div>
        </section>
